...
void runTestSuite_time() {
    ...
    TEST_CASE(test_time_source_names) {
        ...
        TEST_CASE(test_market_config) {
            // 验证市场配置常量
            LOG_ERROR("    MarketConfig::BASE_YEAR: %d (expected: 2000)", MarketConfig::BASE_YEAR);
            LOG_ERROR("    MarketConfig::BASE_MONTH: %d (expected: 1)", MarketConfig::BASE_MONTH);
            LOG_ERROR("    MarketConfig::BASE_DAY: %d (expected: 1)", MarketConfig::BASE_DAY);
            LOG_ERROR("    MarketConfig::CYCLE_DAYS: %d (expected: 3)", MarketConfig::CYCLE_DAYS);

            ASSERT_TRUE(MarketConfig::BASE_YEAR == 2000);
            ASSERT_TRUE(MarketConfig::BASE_MONTH == 1);
            ASSERT_TRUE(MarketConfig::BASE_DAY == 1);
            ASSERT_TRUE(MarketConfig::CYCLE_DAYS == 3);
        }
        TEST_CASE_END();
