...
void initBasicSystem() {
  ...
  Serial.println();
  Serial.println("========================================");
  Serial.printf("ESP8266 SSD1306 Clock %s\n", getVersionString());
  ...
  LOG_INFO("Firmware version: %s", getVersionString());
...
bool checkK4LongPress() {
  ...
  if (pressed) {
    ...
    ButtonState& k4Btn = buttonStates.buttons[3]; // K4 对应索引 3
    ...
    // 重置消抖和处理时间 - 使用过去的时间点，避免与 updateButtonStates() 冲突
    k4Btn.lastDebounceTime = millis() - DEBOUNCE_DELAY - 10;
