...
void displayDate(const DateTime& now) {
  static char dateStr[30]; // 增加缓冲区大小以处理UTF-8中文字符
  formatDateString(dateStr, sizeof(dateStr), now.year(), now.month(), now.day());
  
  // 检查字符串长度是否超过缓冲区
  if (strlen(dateStr) >= sizeof(dateStr) - 1) {
    // 处理缓冲区截断情况
...
void displayTimeValue(const DateTime& now) {
  int hour = now.hour();
  static char timeStr[20]; // 增加缓冲区大小确保安全
  formatTimeString(timeStr, sizeof(timeStr), hour, now.minute(), now.second());
  
  // 检查字符串长度是否超过缓冲区
  if (strlen(timeStr) >= sizeof(timeStr) - 1) {
    // 处理缓冲区截断情况
...
void calculateMarketDay(time_t currentTime, int& marketIndex) {
  ...
  // 使用gmtime_r替代localtime，更安全
  struct tm currentTm;
  if (gmtime_r(&currentTime, &currentTm) == nullptr) {
    marketIndex = 0; 
    return;
  }
  ...
  currentTm.tm_min = 0;
  currentTm.tm_sec = 0;
  time_t currentDayTime = mktime(&currentTm);
  if (currentDayTime < 0) {
    marketIndex = 0; 
    return;
  }
  ...
  // 计算天数差值并计算市场日
  long daysDiff = (currentDayTime - baseTime) / SECS_PER_DAY;
...
void displayBrightnessSettingScreen() {
  ...
  drawProgmemString((const char*)pgm_read_ptr(&BRIGHTNESS_LABELS[displayState.brightnessIndex]), 0, 48);
  ...
  // 绘制已填充部分
  int filledWidth = (barWidth * (displayState.brightnessIndex + 1)) / 4;
  u8g2.drawBox(barX, barY, filledWidth, barHeight);
  
  // 绘制亮度等级指示器
  int indicatorX = barX + (barWidth * displayState.brightnessIndex) / 4;
